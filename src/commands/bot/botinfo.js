  ...
  run ({ channel, author, t }) {
    ...
    embed.setAuthor(this.client.user.username, this.client.user.displayAvatarURL)
      ...
      .setDescription([
        ...
        t('commands:botinfo.statistics', { guilds: this.client.guilds.size, commands: this.client.commands.length, uptime, Discord, nodeVersion: process.version, users: this.client.users.filter(u => !u.bot).size })
      ].join('\n\n'))
