  ...
  async run ({ t, author, channel, message }, flightNum) {
    try {
      ...
      const embed = new SwitchbladeEmbed()
        .setTitle(`${data.identification.callsign} - ${data.aircraft.registration || data.aircraft.model.code} - ${data.aircraft.model.text}`)
        .setURL(`https://fr24.com/${data.identification.id}`)
        .setDescription(`**${t('commands:flightradar24.airline')}:** ${data.airline?.name || 'Unknown'}`)
        ...
        .setTimestamp(null)
      if (data.status.live && data.airport.origin) {
        embed.addField(`**${t('commands:flightradar24.departed')} <t:${data.firstTimestamp}:R>**`, `[${data.airport.origin.name}](${data.airport.origin.website}) (${data.airport.origin.code.iata}/${data.airport.origin.code.icao})\n**${t('commands:flightradar24:locatedAt')}:** ${data.airport.origin.position.region.city} - ${data.airport.origin.position.country.name} (${data.airport.origin.timezone.offsetHours})`, true)
      }
      if (data.status.live && data.airport.destination) {
      ...
      embed.setImage(data.aircraft.images?.large?.[0].src || data.aircraft.images?.sideview || 'https://www.flightradar24.com/static/images/jp-promo-blocked.jpg')
