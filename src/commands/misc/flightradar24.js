  ...
  async run ({ t, author, channel, message }, flightNum) {
    try {
      ...
      const embed = new SwitchbladeEmbed()
        .setTitle(`${airplaneData.identification.callsign} - ${airplaneData.aircraft.registration || airplaneData.aircraft.model.code} - ${airplaneData.aircraft.model.text}`)
        .setURL(`https://fr24.com/${airplaneData.identification.id}`)
        .setDescription(`**${t('commands:flightradar24.airline')}:** ${airplaneData.airline?.name || 'Unknown'}`)
      ...
      if (airplaneData.status.live) {
        if (airplaneData.airport.origin) {
          embed.addField(`**🛫 ${t('commands:flightradar24.departed')} <t:${airplaneData.firstTimestamp}:R>**`, `[${airplaneData.airport.origin.name}](${airplaneData.airport.origin.website}) (${airplaneData.airport.origin.code.iata}/${airplaneData.airport.origin.code.icao})\n**${t('commands:flightradar24:locatedAt')}:** ${airplaneData.airport.origin.position.region.city} - ${airplaneData.airport.origin.position.country.name} (${airplaneData.airport.origin.timezone.offsetHours})`, true)
        }
        if (airplaneData.airport.destination) {
          embed.addField(`🛬 **${t('commands:flightradar24.expectedArrival')} <t:${airplaneData.status.generic.eventTime.utc}:R>**`, `[${airplaneData.airport.destination.name}](${airplaneData.airport.destination.website}) (${airplaneData.airport.destination.code.iata}/${airplaneData.airport.destination.code.icao})\n**${t('commands:flightradar24:locatedAt')}:** ${airplaneData.airport.destination.position.region.city} - ${airplaneData.airport.destination.position.country.name} (${airplaneData.airport.destination.timezone.offsetHours})`, true)
        }
      }
