const {SwitchbladeEmbed, SearchCommand, Constants, CommandStructures} =
    require('../../')
...
module.exports = class Itunes extends SearchCommand {
  ...
  searchResultFormatter(i) {
    if (!i.error) {
      return `[${i.trackName}](${i.trackViewUrl}) - [${i.artistName}](${
          i.artistViewUrl})`
    } else {
      return i.error
    }
  ...
  async handleResult({t, author, channel}, data) {
    ...
    const embed =
        new SwitchbladeEmbed(author)
            ...
            .setAuthor(
                t('commands:itunes.place'), this.embedLogoURL,
                'https://www.apple.com/itunes/')
            .setURL(data.trackViewUrl)
            .setTitle(
                t('commands:itunes.track',
                  {name: data.trackName, artistName: data.artistName}))
            ...
            .setDescriptionFromBlockArray([
              [data.description ? `Description: ${
                   ...
                   data.longDescription ?
                                  ...
                                  ''],
              [
                data.userRatingCount ? t('commands:itunes.ratingCount',
                                         {count: data.userRatingCount || 0}) :
                                       '',
                data.averageUserRating ?
                    `Rating: ${stars} (${
                        Math.round(data.averageUserRating) || ''})` :
                    ''
              ],
              [t('commands:itunes.price', {
                price: data.formattedPrice || data.trackPrice,
                currency: data.currency
              })],
              [t('commands:itunes.release',
                 {date: moment(data.releaseDate).format('DD/MM/YYYY')})],
