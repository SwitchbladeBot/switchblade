...
module.exports = class Serverinvite extends Command {
 constructor(client) {
  ...
  this.parameters = new CommandParameters(this, new StringParameter({
   full: true,
 ...
 async run({
  ...
  message.client.guilds.get(guildID).channels.filter((channel) => channel.permissionsFor(message.guild.me).has(1) && channel.type !== 'category').first().createInvite({
   maxAge: 0
  }).then((invite) => {
