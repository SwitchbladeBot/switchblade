...
module.exports = class LeagueOfLegends extends Command {
  constructor (client) {
    super(client, {
      ...
      parameters: [{
        type: 'string', full: true, missingError: 'commands:leagueoflegends.noChampion'
      }]
    })
  ...
  async run ({ t, author, channel, language }, champion) {
    ...
    try {
      ...
      embed.setColor(this.LOL_COLOR)
        ...
        .addField(t('commands:leagueoflegends.spells'), champ.spells.map((spell, i) => `**${spell.name}** (${buttons[i]})`).join(', '), true)
      channel.send(embed).then(() => channel.stopTyping())
    } catch (e) {
      throw new CommandError(t('commands:leagueoflegends.invalidChamp'))
    }
  }
}
